...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if bot_hibernating:
            cache = hibernation_snapshot.setdefault("stats_cache", {})
            if uid in cache:
                report = cache[uid]
            else:
...
async def cmd_ranking(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if bot_hibernating and hibernation_snapshot.get("ranking") is not None:
            ranking = hibernation_snapshot.get("ranking") or []
        else:
...
if __name__ == "__main__":
    ...
    app.job_queue.run_daily(handle_hibernation_start, time=datetime.strptime("22:00", "%H:%M").time().replace(tzinfo=AR_TZ))
    app.job_queue.run_daily(handle_hibernation_end, time=datetime.strptime("06:00", "%H:%M").time().replace(tzinfo=AR_TZ))
