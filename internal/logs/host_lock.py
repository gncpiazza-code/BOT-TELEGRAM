...
class HostLock:
    ...
    def _get_or_create_host_control_sheet(self):
        ...
        try:
            ...
            logger.info("📄 Creando HOST_CONTROL...")
            ...
            self.sheets._ws_cache["HOST_CONTROL"] = ws
            return ws
            
        except Exception as e:
            logger.error(f"Error HOST_CONTROL: {e}")
            return None
    ...
    def _get_cached_data(self, ws, force_refresh: bool = False):
        ...
        if force_refresh or (now - self._last_read_timestamp) > self.CACHE_TTL:
            try:
                self._last_read_data = ws.get_all_values()
                ...
                if host_hb:
                    try:
                        ...
                        if diff > self.HOST_DEAD_TIMEOUT:
                            logger.info(f"🧹 Host muerto ({diff}s), limpiando...")
                            ws.update("A2:K2", [["", "", "", "", "", "", "", "", "", "", ""]])
                            self._invalidate_cache()
                            return
                    except:
