...
6) ✅ NUEVO: Filtro de Ranking por rango + mes específico detectado en la base (STATS).
   - Opciones: Hoy / 15 días / Mes / {meses detectados} / Histórico.
   - Filtro ubicado en la sección del Ranking ("Seleccionar rango").
   - Compatible con Flet 0.80.1 (usa Dropdown, no controles nuevos).
"""
...
def _month_label(y: int, m: int) -> str:
    return f"{_MONTH_ES.get(m, str(m))} {y}"


def _extract_available_months(rows: List[Dict[str, Any]]) -> List[Tuple[int, int]]:
...
def _build_dropdown_option(value: str, text: str) -> Any:
    dd = _safe_getattr(ft, "dropdown")
    Option = None
    if dd is not None:
        Option = _safe_getattr(dd, "Option")
    Option = Option or _safe_getattr(ft, "Option")
    if Option is None:
        return value  # fallback (muy raro)

    opt_params = _sig_params(Option)
    kwargs: Dict[str, Any] = {}
    if "key" in opt_params:
        kwargs["key"] = value
    if "text" in opt_params:
        kwargs["text"] = text
    try:
...
def main(page: ft.Page) -> None:
    ...
    state: Dict[str, Any] = {
        "periodo": "mes",  # hoy | 15dias | mes | m:YYYY-MM | historico
    ...
    # -------------------------
    # FILTER (RANKING) - Dropdown
    # -------------------------
    periodo_dd: Optional[ft.Dropdown] = None
    ...
    def _period_label(value: str) -> str:
        ...
        if value.startswith("m:"):
            try:
                y, m = value[2:].split("-")
                return _month_label(int(y), int(m))
            except Exception:
                return value
        return value

    def _build_period_options() -> List[Any]:
        opts: List[Any] = [
            _build_dropdown_option("hoy", "Hoy"),
            _build_dropdown_option("15dias", "15 días"),
            _build_dropdown_option("mes", "Mes"),
        ]

        for (y, m) in state.get("available_months", []) or []:
            key = f"m:{_month_key(y, m)}"
            opts.append(_build_dropdown_option(key, _month_label(y, m)))

        opts.append(_build_dropdown_option("historico", "Histórico"))
        return opts

    def _sync_period_dropdown_options() -> None:
        nonlocal periodo_dd
        if periodo_dd is None:
            return
        periodo_dd.options = _build_period_options()

        current = str(state.get("periodo", "mes") or "mes")
        valid_keys = {getattr(o, "key", None) for o in (periodo_dd.options or [])}
        if current not in valid_keys:
            current = "mes"
            state["periodo"] = current

        try:
            periodo_dd.value = current
        except Exception:
            pass

    def on_period_change(e) -> None:
        raw = getattr(getattr(e, "control", None), "value", None)
        chosen = str(raw).strip() if raw else ""
        if not chosen:
            return
        state["periodo"] = chosen
        request_refresh(force=False)

    def build_rank_filter() -> ft.Control:
        nonlocal periodo_dd
        dd_params = _sig_params(ft.Dropdown)
        kwargs: Dict[str, Any] = {
            "options": _build_period_options(),
            "on_change": on_period_change,
        }
        if "value" in dd_params:
            kwargs["value"] = state.get("periodo", "mes")
        if "dense" in dd_params:
            kwargs["dense"] = True
        if "text_size" in dd_params:
            kwargs["text_size"] = 12
        if "border_radius" in dd_params:
            kwargs["border_radius"] = 10
        if "bgcolor" in dd_params:
            kwargs["bgcolor"] = _op(0.10, "#FFFFFF")
        if "color" in dd_params:
            kwargs["color"] = TXT
        if "border_color" in dd_params:
            kwargs["border_color"] = _op(0.35, AMBER)
        if "label" in dd_params:
            kwargs["label"] = "Seleccionar rango"
        elif "hint_text" in dd_params:
            kwargs["hint_text"] = "Seleccionar rango"

        periodo_dd = ft.Dropdown(**kwargs)

        return ft.Container(
            padding=_padding_symmetric(10, 6),
            border_radius=14,
            bgcolor=_op(0.06, "#FFFFFF"),
            border=_border_all(1, _op(0.20, "#FFFFFF")),
            content=ft.Row(
                spacing=10,
                vertical_alignment="center",
                controls=[
                    _guaranteed_icon("CALENDAR_MONTH", color=AMBER, size=16, fallback_text="🗓️"),
                    ft.Container(width=210, content=periodo_dd),
                ],
            ),
        )
    ...
    # -------------------------
    # RANK PANEL (con filtro dentro)
    # -------------------------
    rank_filter = build_rank_filter()
    ...
    def filter_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        valid_states = ("Aprobado", "Destacado", "Rechazado")
        periodo = str(state.get("periodo", "mes") or "mes").strip()
        ...
        if periodo == "15dias":
            start = today - timedelta(days=14)  # ventana inclusiva (hoy + 14 atrás = 15 días)
            out = []
            for r in rows:
                st = str(r.get("ESTADO_AUDITORIA", "")).strip()
                d = _parse_date(_get(r, "FECHA"))
                if st in valid_states and d and start <= d.date() <= today:
                    out.append(r)
            return out
        ...
        if periodo.startswith("m:"):
            try:
                y_s, m_s = periodo[2:].split("-")
                target = (int(y_s), int(m_s))
            except Exception:
                target = None
            ...
            if target is not None:
                ...
                for r in rows:
                    st = str(r.get("ESTADO_AUDITORIA", "")).strip()
                    d = _parse_date(_get(r, "FECHA"))
                    if st in valid_states and d and _month(d) == target:
                        out.append(r)
                return out
    ...
    def compute_snapshot(force: bool) -> Dict[str, Any]:
        ...
        available_months = _extract_available_months(cache)
        rows = filter_rows(cache)
        stats = calc_stats(rows)
        last = _last_ok(rows)
        return {"ok": True, "stats": stats, "last": last, "available_months": available_months}
    ...
    def apply_snapshot(snap: Dict[str, Any]) -> None:
        ...
        # ✅ actualizar meses detectados y refrescar dropdown
        state["available_months"] = snap.get("available_months", []) or []
        _sync_period_dropdown_options()
